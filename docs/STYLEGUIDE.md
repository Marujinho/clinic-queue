...
| Component | Type | Path | Spec | Status | Notes |
|-----------|------|------|------|--------|-------|
| Queue index | Livewire page | `resources/views/pages/queue/⚡index.blade.php` | [§7.6](#76-data-table) | Done | Queue config list (`pages::queue.index`, route `/queues`): name, description, department, priority badge, status, waiting count; Edit + Activate/Deactivate; admin-only (`manage-queues`) |
| Check-in | Livewire page | `resources/views/pages/queue/⚡check-in.blade.php` | [§7](#7-component-specs) | Done | Patient check-in (`pages::queue.check-in`, route `/check-in`): searchable active-patient picker, active queue select, optional open appointment, priority (only when queue priority-enabled); enforces BR-02/BR-05, numbers tickets via `TicketNumberGenerator` |
